ocApp.controller('challengeCtrlPub', function($scope, $routeParams, Restangular, $location, $rootScope, $sce, $timeout, $route, $window) {
	$scope.challenge = {
		pages: [],
		header_images: [],
		submit_fields: [],
	...
	$scope.portUrl = $location.port();
	$scope.urlRoot = $location.protocol() + "://" + $location.host();
	$scope.currentLocation = $location.absUrl();

	//Inits
	$scope.editProjectsInit = function(){
		$scope.checkIfParticipate();
		$scope.checkCanPubEdit();
		if(!$rootScope.user || !$scope.checkIfParticipate){
	...
	$scope.checkIfParticipate = function(){
		if($rootScope.user){
		...
		if($routeParams.challengeId){
			Restangular.one('dashboards', $routeParams.challengeId).get()
				...
				if($scope.isAdmin){
                    ...
                    $scope.projects = Restangular.one('dashboards', $routeParams.challengeId).one($routeParams.userId).getList('myprojects').$object;
				}
	...
	$scope.exportData = function(challengeId, userId){
		if ($scope.portUrl){
			$scope.portUrl = ":"+ $scope.portUrl;
		}

	    //If JSONData is not an object then JSON.parse will parse the JSON string in an Object
	    $.getJSON( $scope.urlRoot + $scope.portUrl + "/api/v2/myprojects/export/" + challengeId + "/" + userId, function( data ) {
	    	JSONData = data;
